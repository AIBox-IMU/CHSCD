...
def get_loaders(opt):
    ...
    val_dataset = CDDloader(opt, 'test', aug=False)
...
class CDDloader(data.Dataset):
    ...
    def __init__(self, opt, phase, aug=False):
        ...
        names = [i for i in os.listdir(
            os.path.join(self.data_dir, phase, 'im1'))]
        self.names = []
        for name in names:
            if is_img(name):
    ...
    def __getitem__(self, index):
        ...
        img_A = Image.open(os.path.join(self.data_dir, self.phase, 'im1', name))
        img_B= Image.open(os.path.join(self.data_dir, self.phase, 'im2', name))
        label_name = name.replace("tif", "png") if name.endswith(
            "tif") else name   # for shengteng

        label_A = Image.open(os.path.join(
            self.data_dir, self.phase, 'label1_rgb', label_name))
        label_B = Image.open(os.path.join(
            self.data_dir, self.phase, 'label2_rgb', label_name))
...
class CDDloadImageOnly(data.Dataset):
    ...
    def __init__(self, opt, phase, aug=False):
        ...
        self.phase = str(phase)
        self.aug = aug
        names = [i for i in os.listdir(os.path.join(self.data_dir, phase, 'im1'))]
        self.names = []
        for name in names:
            if is_img(name):
                self.names.append(name)
    ...
    def __getitem__(self, index):
        name = str(self.names[index])
        img1 = Image.open(os.path.join(self.data_dir, self.phase, 'im1', name))
        img2 = Image.open(os.path.join(self.data_dir, self.phase, 'im2', name))
