...
class CrossEntropyLoss2d(nn.Module):
    '''
    用于图像分割任务的 2D 交叉熵损失函数，即 CrossEntropyLoss2d。
    它继承自 PyTorch 的 nn.Module，并使用了负对数似然损失（NLLLoss）
    来计算像素级分类损失。
    '''
    def __init__(self, weight=None, ignore_index=-1):
        super(CrossEntropyLoss2d, self).__init__()
        '''
        reduction='elementwise_mean' 表示会对损失进行平均（每个样本的损失加权平均）
        进一步扩展这个损失函数，比如加入 Dice Loss、Focal Loss
        '''
...
def weighted_BCE(output, target, weight_pos=None, weight_neg=None):
    '''
    加权二元交叉熵损失函数
    output: 是模型输出的概率值（经过 sigmoid 后的结果）
    target: 是标签（0 或 1）
    weight_pos, weight_neg: 分别是正类和负类的权重（用于处理类别不平衡）
    '''
    output = torch.clamp(output,min=1e-8,max=1-1e-8) # 为了避免对数运算时出现 log(0) 导致 NaN，这里对输出进行了裁剪。这是一个标准做法。
    
    if weight_pos is not None: # 加权机制
...
class ChangeSimilarity(nn.Module):
    """input: x1, x2 multi-class predictions, c = class_num
       ...
       计算两个分类输出之间的变化相似性损失（change similarity loss），主要使用了余弦嵌入损失（Cosine Embedding Loss）
    """
