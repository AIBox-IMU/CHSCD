...
class HGINet(nn.Module):
    ...
    def CD_forward(self, x1, x2):  # 特征交互(两个输入和输出 + concat) 和 特征融合
        ###SDI module
        cz = torch.abs(x2 - x1)
        n,c,h,w = x1.size()
        ...
        change = self.out_feature1(change) # 保留
        return change

    def forward(self, t1, t2):

        t1_out = self.base_forward(t1) # 2
        t2_out = self.base_forward(t2) # 2
        t1_out, t2_out = self.TC_module(t1_out,t2_out) # 3 交互

        output1 = self.out_feature(t1_out)
        output2 = self.out_feature2(t2_out)
        change = self.CD_forward(t1_out,t2_out) # 1 融合
