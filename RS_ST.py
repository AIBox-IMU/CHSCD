...
# root = r'F://yq//chapter4//FZ_SCD//'
root = r'F://yq//chapter4//Landsat_SCD//'
# root = r'/public/home/action117417/chapter4/FZ_SCD/'
# root = r'/public/home/action117417/chapter4/Seond_SCD/'
...
def Color2Index(ColorLabel):
    ...
    IndexMap = np.clip(IndexMap, 0, num_classes - 1).astype(np.uint8)  # 假设 num_classes <= 256
    return IndexMap
...
class Data(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img_A = cv2.resize(img_A, self.target_size, interpolation=cv2.INTER_LINEAR)
        img_A = normalize_image(img_A, 'A')

        img_B = io.imread(self.imgs_list_B[idx])
        # img_B = cv2.resize(img_B, self.target_size, interpolation=cv2.INTER_LINEAR)
        ...
        # label_A = cv2.resize(label_A, self.target_size, interpolation=cv2.INTER_NEAREST)
        # label_B = cv2.resize(label_B, self.target_size, interpolation=cv2.INTER_NEAREST)
...
# tif
def main0():
    # working_dir = r'D://yq//PycharmProject//datasets//Chapter2//FZ_SCD//FZ_SCD//train//'

    SrcDir = os.path.join(root, 'label2')
    DstDir = os.path.join(root, 'label2_rgb')
    ...
    for it in data_list:
        if it.lower().endswith('.tif') or it.lower().endswith('.tiff'):
            img_src_path = os.path.join(SrcDir, it)
            img_dst_path = os.path.join(DstDir, os.path.splitext(it)[0] + '.tif')  # 可选：保存为 .png

            label = io.imread(img_src_path)
            # label = Color2Index(label)  # 如需转换颜色到索引
            label_new = Index2Color(label)  # 假设这是你自己的函数
...
def main1():
    ...
    # SrcDir = os.path.join(root, 'label2') # To use here for converting INDEX to RGB picture
    # DstDir = os.path.join(root, 'label2_rgb')

    SrcDir = os.path.join(root, 'label2_rgb') # To use here for converting RGB to INDEX picture
    DstDir = os.path.join(root, 'label2')
    ...
    for it in data_list:
        if it.lower().endswith('.png'):
            img_src_path = os.path.join(SrcDir, it)
            img_dst_path = os.path.join(DstDir, os.path.splitext(it)[0] + '.png')  # 可选：保存为 .png

            label = io.imread(img_src_path)
            label_new = Color2Index(label)  # 如需转换颜色到索引
            # label_new = Index2Color(label)  # 假设这是你自己的函数
            io.imsave(img_dst_path, label_new)
            print('Process finished: ' + img_dst_path)
